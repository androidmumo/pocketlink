...
当前已发布 OTA `0.5.0`（序号 5）：增加消息发送人、发送时间、北京时间时钟，以及房间半双工对讲。网页新操作区与对讲服务已部署；已完成首次有线安装的设备可在线升级。用户暂不方便测试，0.5.0 真机安装、显示、音质和延迟待验收；此前 0.4.4 升级已获确认。详见 [OTA 教程](docs/development/firmware-ota.md) 和 [对讲用法](docs/development/voice.md)。
...
“创建房间、加入房间、邀请朋友”统一在房间页顶部。房间邀请留在当前房间内，注册邀请码仅在管理员入口管理。手机页面禁止缩放。协议、限制和完整步骤见[对讲教程](docs/development/voice.md)。真实音质与延迟需真机验收。
