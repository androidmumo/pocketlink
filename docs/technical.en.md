[简体中文](technical.md) | English

# Technical and operations reference

For everyday use, start with the [device guide](guides/device.en.md) or [Docker guide](guides/docker.en.md). This reference retains configuration, maintenance and development details. See [release maintenance](development/releases.en.md).
...
**The console now supports invite registration, isolated user accounts, shared rooms and a responsive workspace.** See [accounts and invitations](development/accounts.en.md).
...
[Device provisioning and flashing guide](development/device-provisioning.en.md)
...
OTA `0.5.0` (sequence 5) is published with sender/send-time metadata, a Beijing-time clock and room half-duplex intercom. The new room actions and voice service are deployed. Devices with the initial wired installation can upgrade online. Physical installation, display, audio quality and latency await user acceptance; earlier 0.4.4 acceptance remains confirmed. See the [OTA guide](development/firmware-ota.en.md) and [intercom guide](development/voice.en.md).
...
4. With the pocketlink development firmware, scan the device hotspot QR and enter Wi-Fi
   settings and the pairing code on its local page. See [device provisioning](development/device-provisioning.en.md).
   **Generating a code alone does not add a device**; it appears after successful pairing.
   Developers can also use the [authentication API](development/authentication.en.md).
...
Retry identical content after network failure; check history before resending after reload. See the [text/WSS contract](development/messaging.en.md).
...
The pinned digest comes from the [successful CI run](https://github.com/androidmumo/pocketlink/actions/runs/35940663108).
...
requires ESP-IDF 5.5.3. See [environment setup](development/setup.en.md).
...
- [Architecture and stages](architecture/overview.en.md)
- [Device authentication and API](development/authentication.en.md)
- [Protocol format](../packages/protocol/README.en.md)
- [Deployment record](operations/deployment.en.md)
- [Hardware constraints and provenance](../firmware/boards/ai_passport/README.en.md)
- [Changelog](CHANGELOG.en.md) · [Third-party notices](THIRD_PARTY.en.md)
...
Create, Join and Invite actions share the room page top area. Room invitations stay inside their room; registration invitations are administrator-only. Mobile page zoom is disabled. See the [intercom guide](development/voice.en.md) for steps, protocol and limits. Actual audio quality and latency need device acceptance.
