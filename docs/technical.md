简体中文 | [English](technical.en.md)

# 技术与运维说明

日常使用请先看[设备简单教程](guides/device.md)或[Docker 简单教程](guides/docker.md)。这里保留完整配置、维护命令和开发背景。固件发布方式见[Releases 维护](development/releases.md)。

一个仓库，管理口袋设备固件、中转服务和网页管理端，后续扩展联机游戏与其他应用。

**工作台支持邀请码注册、独立用户空间和跨账号共享房间，保留文字发送、离线补发和逐设备回执。** [账号与邀请教程](development/accounts.md)
新增 **P3 设备开发版**：二维码热点配网、HTTPS 配对、文字收信及签名 OTA，尚待真机验收。
已提供网页与设备半双工对讲开发版，真机音质与延迟待验收。`board-check` 仍只是硬件诊断；设备功能请使用 `pocketlink` 固件。
[设备配网与刷机教程](development/device-provisioning.md)

[功能与使用](#使用管理页) · [部署](#首次部署到新服务器) · [维护与升级](#日常维护与升级) · [备份恢复](#备份与恢复) · [常见问题](#常见问题) · [开发](#本地开发与仓库结构)

## 当前已部署的环境

访问 **[PocketLink 管理页](https://pocketlink.mcloc.cn)**。以下是 2026-09-23 升级并复核的部署记录，后续实际配置以服务器文件为准。
...
当前已发布 OTA `0.5.0`（序号 5）：增加消息发送人、发送时间、北京时间时钟，以及房间半双工对讲。网页新操作区与对讲服务已部署；已完成首次有线安装的设备可在线升级。用户暂不方便测试，0.5.0 真机安装、显示、音质和延迟待验收；此前 0.4.4 升级已获确认。详见 [OTA 教程](development/firmware-ota.md) 和 [对讲用法](development/voice.md)。
...
4. 使用 `pocketlink` 开发固件时，扫码连接设备热点，在本地页面填写 Wi-Fi 和配对码。
   详见[设备配网](development/device-provisioning.md)。**仅生成配对码不会自动添加设备**；
   设备成功联网并提交配对码后才会出现。开发者也可按[鉴权接口](development/authentication.md)联调。
...
网络异常后同一正文可重试，页面重载后重发前先核对历史。详见[文字接口和 WSS 协议](development/messaging.md)。
...
[已通过的 CI 构建](https://github.com/androidmumo/pocketlink/actions/runs/35826339327)，不要把浮动 `edge` 当作固定版本。
...
| 设备没有收信、讲话按钮 | 首次安装需有线刷入；新版固件长按下键进入对讲，需先在网页将设备加入房间 |
...
固件构建另需 ESP-IDF 5.5.3，安装步骤见[环境准备](development/setup.md)。
...
- [架构与阶段计划](architecture/overview.md)
- [设备鉴权及 API](development/authentication.md)
- [协议格式](../packages/protocol/README.md)
- [部署记录](operations/deployment.md)
- [硬件约束与来源](../firmware/boards/ai_passport/README.md)
- [更新记录](CHANGELOG.md) · [第三方声明](THIRD_PARTY.md)
...
已实现半双工对讲开发版：房间内按住讲话、松开收听，支持网页和设备。设备长按下键进入对讲，短按上下切房，长按下键退出；对讲期间暂停文字轮询。消息显示发送人和发送时间，顶部显示北京时间到分钟。

“创建房间、加入房间、邀请朋友”统一在房间页顶部。房间邀请留在当前房间内，注册邀请码仅在管理员入口管理。手机页面禁止缩放。协议、限制和完整步骤见[对讲教程](development/voice.md)。真实音质与延迟需真机验收。
