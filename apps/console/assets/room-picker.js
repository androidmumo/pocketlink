...
(() => {
  ...
  function highlight(){[...list.children].forEach((item,i)=>{item.classList.toggle('highlighted',i===active);});const item=list.children[active];if(item){button.setAttribute('aria-activedescendant',item.id);item.scrollIntoView({block:'nearest'});}}
  ...
  function open(){
    ...
    const rect=button.getBoundingClientRect(),below=innerHeight-rect.bottom-12,above=rect.top-12;
  ...
  document.addEventListener('scroll',e=>{if(!list.contains(e.target))close();},true);
