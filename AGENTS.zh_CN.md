...
首先阅读本文件。范围见[架构](docs/architecture/overview.zh_CN.md)，工具与检查见
[环境准备](docs/development/setup.zh_CN.md)，部署前阅读[运维](docs/operations/deployment.zh_CN.md)。
...
- 不得把后续阶段功能当作已完成。当前范围为 P2 设备鉴权；房间和消息仍待实现。
...
- 使用有界队列、认证身份、房间权限和协议限制。不记录凭证、文字或音频载荷。
  真实密钥不得进入 Git。
- 所有维护的 Markdown 同时提供英文默认文件和简体中文 `.zh_CN.md`，保留双向链接。
- 用户可见变更记入 `docs/CHANGELOG.md` 及中文版。
